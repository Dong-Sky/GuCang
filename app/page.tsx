...
import { SavedPhotoManager } from "@/components/saved-photo-manager";
import { saveAlbum, type AlbumEntry, type AlbumSession } from "@/lib/images/saved-album";
...
function ItemSheet({ client, item, locations, archived, sharedCount, onAlbumSave, onClose, onEdit, onMove, onDelete }: { archived: ImageRow[]; sharedCount: number; onAlbumSave: (entries: AlbumEntry[], session: AlbumSession, report: ProgressReporter) => Promise<void>; client: SupabaseClient; item: ItemView; locations: LocationRow[]; onClose: () => void; onEdit: () => void; onMove: (status: PhysicalStatus, locationId: string | null) => void; onDelete: () => void }) {
  ...
  if (managing) return <div className="sheet-backdrop"><div className="item-sheet"><SavedPhotoManager photos={item.photos} archived={archived} sharedCount={sharedCount} onClose={() => setManaging(false)} onSave={onAlbumSave} /></div></div>;
    ...
    <div className="app-shell">
      ...
      {selectedItem ? <ItemSheet client={client} archived={workspace.images.filter(image => image.item_style_id === selectedItem.style.id && image.deleted_at)} sharedCount={workspace.instances.filter(instance => instance.item_style_id === selectedItem.style.id).length} onAlbumSave={async (entries, session, report) => {
        ...
        try {
          ...
          const updated = workspaceRef.current?.items.find(i => i.instance.id === selectedItem.instance.id);
          if (updated) setSelectedItemState(updated);
        } finally { mutationRef.current = false; }
      }} item={selectedItem} locations={workspace.locations} onClose={() => setSelectedItem(null)} onEdit={() => { setItemForm({ open: true, initial: selectedItem }); setSelectedItem(null); }} onMove={(status, locationId) => void moveItem(selectedItem, status, locationId)} onDelete={() => void deleteItem(selectedItem)} /> : null}
      {feedbackView}
    </div>
