...
export async function loadWorkspace(client: SupabaseClient, household: Household, userId: string): Promise<Workspace> {
  ...
  const [members, locations, ips, categories, series, characters, styles, instances, links, images, locationImages, movementsResult, exportResult] = await Promise.all([
  ...
  return buildWorkspace({
    ...
    images, locationImages, movements: checked(movementsResult), lastExportAt: checked(exportResult)[0]?.created_at ?? null,
  });
}
...
export async function loadStylePatch(client: SupabaseClient, householdId: string, styleId: string): Promise<StylePatch> {
  ...
  const [instances, images, links, charactersResult, ipsResult, categoriesResult, seriesResult, movementsResult] = await Promise.all([
    ...
    client.from("movement_events").select("*,item_instances!inner(item_style_id)").eq("household_id", householdId).eq("item_instances.item_style_id", styleId).order("created_at", { ascending: false }).limit(300),
  ]);
  return { householdId, style, instances, images, links, characters: checked(charactersResult), ips: checked(ipsResult), categories: checked(categoriesResult), series: checked(seriesResult), movements: checked(movementsResult) };
}
