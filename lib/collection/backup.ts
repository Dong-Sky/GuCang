...
export async function makeArchive(session:BackupSession,download:(p:string)=>Promise<Uint8Array>,report:(s:string)=>void) {
 ...
 const cell=(v:unknown)=>{let s=v==null?'':typeof v==='object'?JSON.stringify(v):String(v);if(typeof v==='string'&&/^[=+@\\-\\t\\r]/.test(s))s="'"+s;return '"'+s.replaceAll('"','""')+'"';};
...
export async function inspectArchive(bytes:Uint8Array,householdId:string,current:Snapshot) {
 ...
 for(const f of Object.values(zip.files)){const size=(f as unknown as {_data?:{uncompressedSize?:number}})._data?.uncompressedSize??0;expanded+=size;if(size>30*1024*1024||expanded>300*1024*1024)throw new Error('备份解压体积超过安全限制');if(f.name.includes('..')||f.name.includes('\\\\'))throw new Error('不安全的备份路径');}
 ...
 if(m.version!==2||m.householdId!==householdId)throw new Error('不支持的备份版本或不是当前谷仓的备份');
 if(!m.checksums||typeof m.checksums!=='object'||!m.checksums['data.json'])throw new Error('校验清单不完整');
