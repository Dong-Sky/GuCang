...
// Newlines delimit selections without splitting historical names containing spaces.
export function characterNames(value: string): string[] {
  return [...new Map(value.split("\n").map(v => v.trim()).filter(Boolean).map(v => [v.normalize("NFKC").toLocaleLowerCase(), v])).values()];
}
