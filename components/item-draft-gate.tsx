...
export function ItemDraftGate({ storageKey, children }: { storageKey: string; children: (draft: ItemDraft | undefined) => ReactNode }) {
  ...
  if (!state.ready || (state.draft && !accepted)) return <div className="sheet-backdrop"><section className="route-error-card" role="dialog" aria-modal="true" aria-label="本地草稿">
    {!state.ready ? <p>正在检查本地草稿…</p> : <><h2>继续上次未保存的内容？</h2><p>仅保留在这个浏览器，包含照片和排序，尚不代表已入库。</p><button className="primary-button" onClick={() => setAccepted(true)}>继续草稿</button><button className="secondary-button" onClick={async () => {
    ...
    }}>放弃草稿，重新填写</button></>}{state.error ? <p role="alert">{state.error}</p> : null}
  </section></div>;
