...
const names: Record<BatchField, string> = { ip: "补 IP", category: "补品类", series: "补系列", character: "补角色", location: "补位置", move: "统一收纳到指定位置", return: "归位到各自默认位置" };
...
export function BatchEditor({ items, workspace, targetLocationId, onClose, onRun }: { items: ItemView[]; workspace: Workspace; targetLocationId?: string; onClose: () => void; onRun: BatchRunner }) {
  const [candidates] = useState(items);
  const [selected, setSelected] = useState<Set<string>>(() => new Set());
  const [search, setSearch] = useState("");
  const [missing, setMissing] = useState<MissingFilter>("");
  const [action, setAction] = useState<BatchAction>(() => targetLocationId ? { field: "move", value: targetLocationId } : { field: "category", value: "" });
  ...
  const isLocation = action.field === "location" || action.field === "move";
  const summary = action.field === "return" ? names.return : `${names[action.field]}：${isLocation ? locationPath(action.value, workspace.locations) : available.find((row) => row.id === action.value)?.name ?? "未选择"}`;
  ...
  return <div className="sheet-backdrop"><section className="batch-sheet" role="dialog" aria-modal="true" aria-labelledby="batch-title">
    ...
    {!confirming ? <>
      <p className="form-hint">每次最多 {BATCH_LIMIT} 件。补资料只补空白；统一收纳会改变当前位置。没有批量删除。</p>
      {targetLocationId ? <p className="entry-notice">正在整理：{locationPath(targetLocationId, workspace.locations)}。从整个谷仓选择要放到这里的收藏。</p> : null}
      ...
      <div className="batch-action-fields"><label>操作<select disabled={Boolean(targetLocationId)} value={action.field} onChange={(e) => setAction({ field: e.target.value as BatchField, value: "" })}>{Object.entries(names).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>{action.field !== "return" ? <label>填入内容<select aria-label="批量填入内容" disabled={Boolean(targetLocationId)} value={action.value} onChange={(e) => setAction({ ...action, value: e.target.value })}><option value="">请选择</option>{available.filter((row) => !row.deleted_at).map((row) => <option key={row.id} value={row.id}>{isLocation ? locationPath(row.id, workspace.locations) : `${row.name}${"ip_id" in row && row.ip_id ? ` · ${workspace.ips.find((ip) => ip.id === row.ip_id)?.name ?? ""}` : ""}`}</option>)}</select></label> : null}</div>
      ...
      {action.field === "move" ? <p className="entry-notice">确认后，所选收藏的当前位置改为目标位置，状态改为已收纳（包括取出中、展示中的收藏）。默认归位位置保持不变。</p> : null}
      <div className="batch-rows" aria-live="polite">{planned.map(({ item, reason }) => <div className="batch-result" key={item.instance.id}><strong>{inventoryCode(item)} · {itemTitle(item)}</strong>{action.field === "move" ? <small>{locationPath(item.instance.current_location_id, workspace.locations)} → {locationPath(action.value, workspace.locations)} · 将设为已收纳</small> : null}<small>{results[item.instance.id]?.message ?? reason ?? (busy ? "等待处理" : "将更新")}</small></div>)}</div>
